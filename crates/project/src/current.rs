...
pub fn parse_config_file(path: Arc<PathBuf>) -> Result<ProjectConfig, ConfigErr> {
    ...
    let contents = std::fs::read_to_string(&buf)
        .map_err(|err| ConfigErr::IoErr(err))?;
    let parsed: ProjectConfig = toml::from_str(&*contents)
        .map_err(|_| ConfigErr::ParseErr)?;
    Ok(parsed)
}
...
pub fn locate_config_file(seed: Option<Arc<PathBuf>>) -> Option<PathBuf> {
    ...
    let mut start = seed.unwrap_or_else(|| Arc::new(PathBuf::from(".")))
        .to_path_buf();
    loop {
...
pub fn resolve_config(seed: Option<Arc<PathBuf>>) -> Result<Option<ProjectConfig>, ConfigErr> {
    match locate_config_file(seed) {
        Some(path) => parse_config_file(Arc::new(path))
            .and_then(|config| Ok(Some(config))),
        None => Ok(None),
    }
}
