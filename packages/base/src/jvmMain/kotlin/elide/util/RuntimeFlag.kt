...
@Suppress("unused", "MemberVisibilityCanBePrivate", "RedundantVisibilityModifier") public object RuntimeFlag {
  ...
  public val appEnv: AppEnvironment
    get() = resolve("elide.appEnv", AppEnvironment.LIVE.name) {
      AppEnvironment.valueOf(it.trim().uppercase())
    }

  /** Whether to enable VM inspection. */
  public val inspect: Boolean
    get() = resolve("elide.vm.inspect", "false") {
      it.trim().toBoolean()
    }

  /** Host where inspection should mount. */
  public val inspectHost: String
    get() = resolve("elide.vm.inspect.host", "localhost") {
      it.trim().ifBlank {
        "localhost"
      }
    }
  ...
  public val inspectPort: Int
    get() = resolve("elide.vm.inspect.port", "4200") {
      it.trim().toIntOrNull() ?: DEFAULT_INSPECT_PORT
    }

  /** Path where inspection should mount. */
  public val inspectPath: String?
    get() = resolve("elide.vm.inspect.path", "") {
      it.trim().ifBlank {
        null
      }
    }
  ...
  public val inspectSuspend: Boolean
    get() = resolve("elide.vm.inspect.suspend", "false") {
      it.trim().toBoolean()
    }

  /** Whether to show internal sources in the inspector. */
  public val inspectInternal: Boolean
    get() = resolve("elide.vm.inspect.internal", "false") {
      it.trim().toBoolean()
    }

  /** Whether to wait for the debugger to attach before executing. */
  public val inspectWait: Boolean
    get() = resolve("elide.vm.inspect.wait", "false") {
      it.trim().toBoolean()
    }

  /** Whether to enable VM inspection secure mode (TLS). */
  public val inspectSecure: Boolean
    get() = resolve("elide.vm.inspect.secure", "false") {
      it.trim().toBoolean()
    }

  /** Whether to enable VM pre-warming. */
  public val warmup: Boolean
    get() = resolve("elide.vm.prewarm", "true") {
      it.trim().toBoolean()
    }
