...
const SNIPPETS: Array<{
  ...
  {
    key: "python",
    title: "Use your existing Python WSGI applications",
    subtitle: "Serve React markup from the WSGI apps you already have",
    language: "python",
    code: PYTHON_SNIPPET,
  },
  {
    ...
    <html lang="en">
      ...
      <body>
        <header>
          ...
          <h1>React SSR + WSGI delivered by Elide</h1>
          <p className="tagline">
            This app renders entirely on the server, it is written using Flask
            and is served by Elide's new WSGI server engine
          </p>
        </header>
...
export default function render({ user_agent }): string {
  return renderToString(
    <App
      nowISO={new Date().toISOString()}
      ua={user_agent ?? ""}
