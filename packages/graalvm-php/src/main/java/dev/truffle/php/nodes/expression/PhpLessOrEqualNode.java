...
/** Node for less-than-or-equal comparison (<=) in PHP. */
@NodeChild("left")
@NodeChild("right")
public abstract class PhpLessOrEqualNode extends PhpExpressionNode {

  @Child protected PhpExpressionNode left;
  @Child protected PhpExpressionNode right;

  protected PhpLessOrEqualNode(PhpExpressionNode left, PhpExpressionNode right) {
    this.left = left;
    this.right = right;
  }

  public static PhpLessOrEqualNode create(PhpExpressionNode left, PhpExpressionNode right) {
    return new PhpLessOrEqualNodeImpl(left, right);
  }

  static final class PhpLessOrEqualNodeImpl extends PhpLessOrEqualNode {
    PhpLessOrEqualNodeImpl(PhpExpressionNode left, PhpExpressionNode right) {
      super(left, right);
    }
