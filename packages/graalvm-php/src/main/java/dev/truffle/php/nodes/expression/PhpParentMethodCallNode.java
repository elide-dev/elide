 ...
 *
 * This node is used when code inside a class method calls a method on the parent class
...
public final class PhpParentMethodCallNode extends PhpExpressionNode {
    ...
    public PhpParentMethodCallNode(String methodName, PhpExpressionNode[] argumentNodes, String currentClassName) {
    ...
    public Object execute(VirtualFrame frame) {
        ...
        if (parentClass == null) {
            throw new RuntimeException("Cannot access parent:: when class " + currentClassName + " has no parent");
        }

        // Check if the parent class has the method
        if (!parentClass.hasMethod(methodName)) {
            throw new RuntimeException("Call to undefined method: " + parentClass.getName() + "::" + methodName + "()");
        }
