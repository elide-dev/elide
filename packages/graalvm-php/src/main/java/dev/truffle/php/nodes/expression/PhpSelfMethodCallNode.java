 ...
 *
 * This node is used when code inside a class method calls a method on the same class
 * using the self:: keyword. This is typically used for static methods, but can also
 * be used to call instance methods in a static context.
 */
public final class PhpSelfMethodCallNode extends PhpExpressionNode {
    ...
    public PhpSelfMethodCallNode(String methodName, PhpExpressionNode[] argumentNodes, String currentClassName) {
    ...
    public Object execute(VirtualFrame frame) {
        ...
        if (!currentClass.hasMethod(methodName)) {
            throw new RuntimeException("Call to undefined method: " + currentClassName + "::" + methodName + "()");
        }
        ...
        if (method.isStatic()) {
            ...
            if (!(thisObject instanceof PhpObject)) {
                throw new RuntimeException("self:: can only be used in object context for non-static methods");
            }
