...
/**
 * AST node for include statement.
 * Includes and executes a PHP file. Returns false if file not found (warning, not fatal error).
 */
public final class PhpIncludeNode extends PhpStatementNode {

    @Child
    private PhpExpressionNode pathNode;
    ...
    public void executeVoid(VirtualFrame frame) {
        ...
        if (!file.exists()) {
            ...
            System.err.println("Warning: include(" + path + "): Failed to open stream: No such file or directory");
            return;
        }
        ...
        try {
            ...
            Source source = Source.newBuilder(PhpLanguage.ID, content.toString(), file.getName())
                .uri(file.toUri())
                .build();
