...
import dev.truffle.php.nodes.expression.PhpArrayAccessNode;
import dev.truffle.php.nodes.expression.PhpFunctionCallNode;
...
/**
 * Node for PHP reference assignment with expressions.
 * Syntax: $b =& $a or $ref =& $arr[1] or $ref =& functionCall()
 *
 * Creates an alias so the target variable references the same value as the source expression.
 * Supports:
 * - Variable to variable: $b =& $a
 * - Array element to variable: $ref =& $arr[1]
...
public final class PhpReferenceAssignmentExprNode extends PhpStatementNode {
    ...
    @Child
    private PhpExpressionNode sourceExpr;

    public PhpReferenceAssignmentExprNode(String targetName, int targetSlot, PhpExpressionNode sourceExpr) {
    ...
    public void executeVoid(VirtualFrame frame) {
        ...
        if (sourceExpr instanceof PhpReadVariableNode) {
            ...
            throw new RuntimeException("Only variables, array elements, and function calls can be assigned by reference");
        }
    }
    ...
    private void handleFunctionCallReference(VirtualFrame frame, PhpFunctionCallNode functionCallNode) {
        ...
        if (result instanceof PhpReference) {
            ...
            throw new RuntimeException("Only functions that return by reference can be assigned by reference");
        }
    }
}
