...
import elide.lang.php.nodes.PhpClosureRootNode;
import elide.lang.php.nodes.PhpExpressionNode;
import elide.lang.php.nodes.PhpNodeFactory;
...
/**
 * Expression parser for PHP, extracted from the main PhpParser. Handles all expression parsing
 * including operators, literals, variables, and function calls.
 */
public final class PhpExpressionParser {
  ...
  private PhpExpressionNode parsePrimary() {
    ...
    if (peek() == '$') {
      ...
      while (match("->")) {
        ...
        if (match("(")) {
          ...
          varNode =
              new PhpMethodCallNode(
                  varNode,
                  member,
                  args.toArray(new PhpExpressionNode[0]),
    ...
    throw new RuntimeException(
        "Unexpected character at position " + lexer.getPosition() + ": " + peek());
  }
  ...
  private PhpExpressionNode parseFullyQualifiedName() {
    ...
    if (match("(")) {
      ...
      if (peek() == '$') {
        ...
        return new PhpStaticMethodCallNode(
            fullyQualifiedName, methodName, args.toArray(new PhpExpressionNode[0]));
      }
    } else {
      ...
      if (peek() == '$') {
        ...
        if (match("(")) {
          ...
          if (identifier.equals("parent")) {
            ...
            return new PhpParentMethodCallNode(
                memberName, args.toArray(new PhpExpressionNode[0]), context.currentClassName);
          }
          ...
          if (identifier.equals("self")) {
            ...
            return new PhpSelfMethodCallNode(
                memberName, args.toArray(new PhpExpressionNode[0]), context.currentClassName);
          }

          return new PhpStaticMethodCallNode(
              identifier, memberName, args.toArray(new PhpExpressionNode[0]));
        } else {
    ...
    switch (name) {
      case "__FILE__":
        return new PhpMagicConstantNode(
            PhpMagicConstantNode.MagicConstantType.FILE, context.sourcePath);
      case "__DIR__":
        ...
        return new PhpMagicConstantNode(PhpMagicConstantNode.MagicConstantType.DIR, dir);
      case "__LINE__":
        // Get current line number from lexer
        long currentLine = (long) lexer.getCurrentLine();
        return new PhpMagicConstantNode(PhpMagicConstantNode.MagicConstantType.LINE, currentLine);
      case "__CLASS__":
        String className = context.currentClassName != null ? context.currentClassName : "";
        return new PhpMagicConstantNode(
            PhpMagicConstantNode.MagicConstantType.CLASS_NAME, className);
      case "__METHOD__":
        String methodName = "";
        ...
        return new PhpMagicConstantNode(PhpMagicConstantNode.MagicConstantType.METHOD, methodName);
      case "__FUNCTION__":
        String funcName = context.currentFunctionName != null ? context.currentFunctionName : "";
        return new PhpMagicConstantNode(PhpMagicConstantNode.MagicConstantType.FUNCTION, funcName);
      case "__NAMESPACE__":
        String namespace = context.currentNamespace != null ? context.currentNamespace : "";
        return new PhpMagicConstantNode(
            PhpMagicConstantNode.MagicConstantType.NAMESPACE, namespace);
      case "__TRAIT__":
        // Traits not yet implemented - return empty string
        return new PhpMagicConstantNode(PhpMagicConstantNode.MagicConstantType.TRAIT, "");
      default:
  ...
  public PhpExpressionNode parseArrayLiteral() {
    ...
    if (isAssociative) {
      return new PhpArrayLiteralNode(
          keys.toArray(new PhpExpressionNode[0]), values.toArray(new PhpExpressionNode[0]));
    } else {
    ...
    while (!isAtEnd() && peek() != quote) {
      if (peek() == '\\') {
        ...
        if (!isAtEnd()) {
          ...
          && peekNext() != '\0'
          && (Character.isLetter(peekNext()) || peekNext() == '_')) {
  ...
  /**
   * Parse function arguments, supporting spread operator (...$expr). Returns a list of expression
   * nodes, which may include PhpSpreadArgumentNode instances.
   */
  ...
  /**
   * Parse a closure (anonymous function). Syntax: function($param1, $param2) use ($captured1,
   * $captured2) { body }
   */
  private PhpExpressionNode parseClosure() {
    ...
    for (int i = 0; i < capturedVarNames.size(); i++) {
      capturedSlots[i] =
          closureFrameBuilder.addSlot(FrameSlotKind.Illegal, capturedVarNames.get(i), null);
    }
    ...
    PhpClosureRootNode closureRoot =
        new PhpClosureRootNode(
            language,
    ...
    return new PhpClosureNode(
        closureRoot.getCallTarget(), paramNames.toArray(new String[0]), capturedExpressions);
  }

  /**
   * Parse an arrow function. Syntax: fn($param1, $param2) => expression Arrow functions
   * automatically capture all variables used in the expression.
   */
  private PhpExpressionNode parseArrowFunction() {
    ...
    for (int i = 0; i < capturedVarNames.size(); i++) {
      capturedSlots[i] =
          arrowFrameBuilder.addSlot(FrameSlotKind.Illegal, capturedVarNames.get(i), null);
    }
    ...
    PhpStatementNode body = new elide.lang.php.nodes.statement.PhpReturnNode(expression);

    // Create arrow function root node
    PhpClosureRootNode arrowRoot =
        new PhpClosureRootNode(
            language,
    ...
    // Return closure node
    return new PhpClosureNode(
        arrowRoot.getCallTarget(), paramNames.toArray(new String[0]), capturedExpressions);
  }

  /** Parse optional type hint (for parameters). Returns silently if no type hint is present. */
