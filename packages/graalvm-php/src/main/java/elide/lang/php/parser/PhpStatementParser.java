...
import elide.lang.php.nodes.statement.*;
 ...
 *
 * <p>This parser handles: - Control flow: if/else, while, for, foreach, switch - Statements: echo,
 * return, throw, try/catch/finally, break, continue - Assignments: variables, arrays, properties,
 * static properties - Blocks: statement grouping with {} - File inclusion: require, include (with
 * _once variants)
 */
public final class PhpStatementParser {

  /**
   * Callback interface for parsing constructs that are defined elsewhere (classes, interfaces,
  ...
  /**
   * Set the namespace context for parsing. Called by PhpParser to provide access to namespace
   * resolution.
   */
  ...
  public PhpStatementNode parseStatement() {
    ...
    if (matchKeyword("abstract")) {
      ...
      if (matchKeyword("class")) {
        ...
        throw new RuntimeException(
            "Expected 'class' after 'abstract' keyword at position " + lexer.getPosition());
      }
    }
    ...
    throw new RuntimeException(
        "Unexpected token at position " + lexer.getPosition() + ": " + peek());
  }
  ...
  private PhpStatementNode parseTry() {
    ...
    while (matchKeyword("catch")) {
      ...
      catchClauses.add(new PhpTryNode.PhpCatchClause(className, varName, varSlot, catchBody));
    ...
    return new PhpTryNode(
        tryBody, catchClauses.toArray(new PhpTryNode.PhpCatchClause[0]), finallyBody);
  }
  ...
  private PhpStatementNode parseSwitch() {
    ...
    while (!check("}") && !isAtEnd()) {
      if (matchKeyword("case")) {
        ...
        cases.add(
            new PhpSwitchNode.CaseClause(
                caseValue, caseStatements.toArray(new PhpStatementNode[0]), false));
        ...
        cases.add(
            new PhpSwitchNode.CaseClause(
                null, defaultStatements.toArray(new PhpStatementNode[0]), true));
      } else {
        throw new RuntimeException(
            "Expected 'case' or 'default' in switch statement at position " + lexer.getPosition());
      }
      skipWhitespace();
    }
    ...
    return new PhpSwitchNode(
        switchExpr, cases.toArray(new PhpSwitchNode.CaseClause[0]), defaultCaseIndex);
  }
  ...
  private PhpStatementNode parseAssignment() {
    ...
    if (match("[")) {
      ...
      if (match("->")) {
        ...
        PhpExpressionNode writeExpr =
            new PhpPropertyWriteNode(arrayAccess, propertyName, value, context.currentClassName);
        return new PhpExpressionStatementNode(writeExpr);
      }
    ...
    if (match("->")) {
      ...
      if (compoundOp != null) {
        ...
        PhpExpressionNode readExpr =
            new PhpPropertyAccessNode(varNode, propertyName, context.currentClassName);
        ...
        PhpExpressionNode writeExpr =
            new PhpPropertyWriteNode(varNode, propertyName, newValue, context.currentClassName);
        return new PhpExpressionStatementNode(writeExpr);
      }
      ...
      PhpExpressionNode writeExpr =
          new PhpPropertyWriteNode(varNode, propertyName, value, context.currentClassName);
      return new PhpExpressionStatementNode(writeExpr);
    }
    ...
    if (compoundOp != null) {
      ...
      PhpExpressionNode compoundExpr =
          new PhpCompoundAssignmentNode(varName, slot, compoundOp, rightValue);
  ...
  private int getOrCreateVariable(String name) {
    ...
    if (context.currentFrameBuilder != null) {
      ...
      return variables.computeIfAbsent(
          name,
          n -> {
            return context.currentFrameBuilder.addSlot(
                com.oracle.truffle.api.frame.FrameSlotKind.Illegal, n, null);
          });
    } else {
  ...
  /** Parse namespace declaration. Syntax: namespace MyApp\Controllers; */
  ...
  /**
   * Parse use statement. Syntax: use MyApp\Models\User; use MyApp\Models\User as UserModel; use
   * function MyApp\Functions\helper; use const MyApp\Constants\VERSION;
   */
  ...
  /** Parse const declaration. Syntax: const CONSTANT_NAME = value; */
