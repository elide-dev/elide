...
import elide.lang.php.nodes.statement.PhpTraitNode;
...
import elide.lang.php.runtime.PhpTrait;
...
/**
 * Parser for PHP trait declarations. Handles trait definitions including methods, properties, and
 * nested trait usage.
 */
public final class PhpTraitParser {
  ...
  public static final class TraitParserContext {
    ...
    public final Map<String, List<String>> traitUsedTraits =
        new HashMap<>(); // traitName -> List<usedTraitNames>
    public final Map<PhpTrait, String> traitNamespaces =
        new HashMap<>(); // Track namespace for each trait
  ...
  public PhpStatementNode parseTrait() {
    ...
    while (!check("}") && !isAtEnd()) {
      ...
      if (matchKeyword("function")) {
        ...
        // For traits, we always use PhpMethodRootNode (even for static) since traits are composed
        // into classes
        ...
        throw new RuntimeException(
            "Unexpected token in trait body at position " + lexer.getPosition());
      }
  ...
  /** Parse optional type hint (for parameters). Currently just skips type hints for traits. */
  ...
  /** Parse required type hint (for return types). Currently just skips type hints for traits. */
