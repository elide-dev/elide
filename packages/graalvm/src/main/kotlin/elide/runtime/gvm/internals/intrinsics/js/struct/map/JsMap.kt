...
@Suppress("unused")
internal class JsMap<K : Any, V> constructor(backingMap: Map<K, V>) :
  ...
  @Suppress("unused")
  internal companion object Factory : MapFactory<JsMap<*, *>> {
     ...
     */
    @JvmStatic override fun <K : Any, V> copyOf(map: Map<K, V>) = JsMap(
     ...
     */
    @JvmStatic override fun <K : Any, V> fromPairs(pairs: Collection<Pair<K, V>>) = JsMap(
      mapImpl<K, V>().apply {
      pairs.forEach {
     ...
     * @return Created JS map instance.
     */
    @JvmStatic override fun <K : Any, V> unbounded(entries: Iterable<MapLike.Entry<K, V>>) = JsMap(
      entries.associate {
      it.key to it.value
    },
    )
    ...
    @JvmStatic
    fun <K : Any, V> empty(size: Int): JsMap<K, V> = JsMap(mapImpl(size))
    ...
    @Suppress("UNCHECKED_CAST")
    @JvmStatic
    override fun <K : Any, V> empty(): JsMap<K, V> = EMPTY_MAP as JsMap<K, V>
  }
