...
internal sealed class NodeBufferInstance : BufferInstance, ProxyObject {
   ...
   * use the same helper.
   */
  protected inline fun readVarInt(offset: Int, byteLength: Int, signed: Boolean = true, getByte: (Int) -> Byte): Long {
    ...
    for (i in 0 until byteLength) {
      value = (value shl 8) or (getByte(i).toLong() and 0xFF)
    }

    if (signed && (value and (1L shl (byteLength * 8 - 1)) != 0L)) {
      // check for sign bit and fill all leading bits with 1 if negative
      value = value or (Long.MAX_VALUE shl (byteLength * 8))
    }
  ...
  override fun getMember(key: String?): Any = when (key) {
    ...
    // note that in the following methods we use a lossy conversion between BigInteger and Long;
    // this is intentional as these methods only accept 64-bit integers
    "writeBigInt64BE" -> ProxyExecutable { args ->
      check(args.isNotEmpty()) { "Expected at least one argument" }
      writeBigInt64BE(args[0].asBigInteger().toLong(), args.getOrNull(1)?.asInt())
    }

    "writeBigInt64LE" -> ProxyExecutable { args ->
      check(args.isNotEmpty()) { "Expected at least one argument" }
      writeBigInt64LE(args[0].asBigInteger().toLong(), args.getOrNull(1)?.asInt())
    }

    "writeBigUInt64BE" -> ProxyExecutable { args ->
      check(args.isNotEmpty()) { "Expected at least one argument" }
      writeBigUInt64BE(args[0].asBigInteger().toLong(), args.getOrNull(1)?.asInt())
    }

    "writeBigUInt64LE" -> ProxyExecutable { args ->
      check(args.isNotEmpty()) { "Expected at least one argument" }
      writeBigUInt64LE(args[0].asBigInteger().toLong(), args.getOrNull(1)?.asInt())
    }
    ...
    "writeUInt8" -> ProxyExecutable { args ->
      check(args.isNotEmpty()) { "Expected at least one argument" }
      writeUInt8(args[0].asInt(), args.getOrNull(1)?.asInt())
    }

    "writeUInt16BE" -> ProxyExecutable { args ->
      check(args.isNotEmpty()) { "Expected at least one argument" }
      writeUInt16BE(args[0].asInt(), args.getOrNull(1)?.asInt())
    }

    "writeUInt16LE" -> ProxyExecutable { args ->
      check(args.isNotEmpty()) { "Expected at least one argument" }
      writeUInt16LE(args[0].asInt(), args.getOrNull(1)?.asInt())
    }

    "writeUInt32BE" -> ProxyExecutable { args ->
      check(args.isNotEmpty()) { "Expected at least one argument" }
      writeUInt32BE(args[0].asLong(), args.getOrNull(1)?.asInt())
    }

    "writeUInt32LE" -> ProxyExecutable { args ->
      check(args.isNotEmpty()) { "Expected at least one argument" }
      writeUInt32LE(args[0].asLong(), args.getOrNull(1)?.asInt())
    }
