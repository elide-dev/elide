...
import elide.runtime.intrinsics.js.node.childProcess.ForkDefaults.ENCODING
import elide.runtime.intrinsics.js.node.childProcess.ForkDefaults.KILL_SIGNAL
import elide.runtime.intrinsics.js.node.childProcess.ForkDefaults.MAX_BUFFER_DEFAULT
import elide.runtime.intrinsics.js.node.childProcess.ForkDefaults.SERIALIZATION
import elide.runtime.intrinsics.js.node.childProcess.ForkDefaults.WINDOWS_HIDE

// Default values used in `fork`.
internal data object ForkDefaults {
 ...
 * ## Fork Options
 *
 * Defines options for the [ChildProcessAPI.fork] method, which is used to spawn forked child processes running the main
 * runtime engine; these options extend [ProcOptions].
 *
...
@API @JvmRecord public data class ForkOptions private constructor (
  ...
  public companion object {
    ...
    public val DEFAULTS: ForkOptions by lazy { ForkOptions(ProcessHandle.current()) }

    /** @return [ForkOptions] created by hand. */
    ...
    /** @return [ForkOptions] derived from a guest [Value]. */
    @JvmStatic public fun from(parent: ProcessHandle, other: Value?): ForkOptions = when (other) {
      null -> DEFAULTS
      else -> when {
        other.isNull -> DEFAULTS
        other.isHostObject -> other.asHostObject()
        other.hasMembers() -> ForkOptions(
          parent = parent,
        ...
        else -> throw JsError.typeError("Invalid type provided as `fork` options")
      }
    }
  }
}
