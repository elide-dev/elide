...
import elide.runtime.intrinsics.js.node.childProcess.SpawnSyncDefaults.WINDOWS_HIDE

// Default values used in `spawnSync`.
internal data object SpawnSyncDefaults {
  ...
  const val KILL_SIGNAL: String = ChildProcessDefaults.SIGNAL_SIGTERM
 ...
 */
@API @JvmRecord public data class SpawnSyncOptions private constructor (
  ...
  /** Factories and other helpers for [SpawnSyncOptions]. */
  public companion object {
    /** Default values for [SpawnSyncOptions]. */
    public val DEFAULTS: SpawnSyncOptions = SpawnSyncOptions()

    /** @return [SpawnSyncOptions] created by hand. */
    ...
    /** @return [SpawnSyncOptions] derived from a guest [Value]. */
    @JvmStatic public fun from(other: Value?): SpawnSyncOptions = when (other) {
      null -> DEFAULTS
      else -> when {
        ...
        else -> throw JsError.typeError("Invalid type provided as `spawnSync` options")
      }
    }
  }
}
