...
internal class NodeUtil private constructor (private val exec: GuestExecutorProvider) : ReadOnlyProxyObject, UtilAPI {
  ...
  override fun getMember(key: String?): Any? = when (key) {
    ...
    F_INSPECT -> ProxyExecutable { args ->
      when (args.size) {
        0 -> inspect(Undefined.instance)
        1 -> inspect(args.first())
        else -> inspect(args.first(), args[1])
      }
    }
    ...
    // F_STRIP_VT_CONTROL_CHARACTERS,
    // F_STYLE_TEXT,
    // F_TO_USV_STRING -> ProxyExecutable { TODO("Not yet implemented: `util.$key`") }
