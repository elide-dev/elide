...
public interface Secrets {
  ...
  /** Returns names of all profiles on a remote, connecting to a remote if a connection has not been made. */
  public suspend fun getRemoteProfiles(): List<String>

  /**
   * Updates specified [profiles] on the local by downloading them from a remote, connecting to a remote if a connection
   * has not been made. If [profiles] is empty, downloads all profiles on the remote.
   */
  public suspend fun updateLocal(vararg profiles: String)

  /**
   * Uploads specified [profiles] to a remote, connecting to a remote if a connection has not been made. If [profiles]
  ...
  /**
   * Returns a secret value with [name] from the selected profile, or throws an [IllegalStateException] if a profile has
   * not been selected.
   */
  ...
  /**
   * Sets a secret to the selected profile, overwriting any existing secret with the same name, or throws an
   * [IllegalStateException] if a profile has not been selected.
   */
  public suspend fun setSecret(secret: Secret<*>)

  /**
   * Removes a secret value with [name] from the selected profile, or throws an [IllegalStateException] if a profile has
   * not been selected.
   */
  public suspend fun removeSecret(name: String)

  /**
   * Writes changes to the selected profile, or throws an [IllegalStateException] if a profile has not been selected.
   */
  ...
  /** Removes the specified profile from a remote, connecting to a remote if a connection has not been made. */
  ...
  public companion object {
    ...
    public suspend inline fun <reified T> Secrets.getTypedSecret(name: String): T? = getSecret(name) as T?

    /** Returns the return value of [getSecret] cast to the reified type [T]. */
    public suspend inline fun <reified T> Secrets.getTypedSecret(profile: String, name: String): T? =
      getSecret(profile, name) as T?
  }
}
