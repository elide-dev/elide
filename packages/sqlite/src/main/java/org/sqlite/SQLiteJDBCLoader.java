...
public class SQLiteJDBCLoader {
    ...
    private static void loadSQLiteNativeLibrary() throws Exception {
        if (extracted) {
          logger.trace("SQLite library already loaded");
          return;
        }
        ...
        String sqliteNativeResourcePath = LibraryLoaderUtil.getNativeLibResourcePath();
        boolean hasNativeLib =
                LibraryLoaderUtil.hasNativeLib(sqliteNativeResourcePath, sqliteNativeLibraryName);

        if (hasNativeLib) {
            logger.trace("Resolving SQLite library from resources");

            // temporary library folder
            String libTargetFolder = Objects.requireNonNullElseGet(
            ...
            if (extractAndLoadLibraryFile(
                    sqliteNativeResourcePath, sqliteNativeLibraryName, libTargetFolder)) {
                logger.trace("SQLite loaded via JNI (attempt 2)");
                extracted = true;
                return;
            } else {
                triedPaths.add(sqliteNativeLibraryPath);
        ...
        logger.trace("Scanning library path for SQLite lib");
