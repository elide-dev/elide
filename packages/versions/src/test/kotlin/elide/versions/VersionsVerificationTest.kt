...
class VersionsVerificationTest : AbstractVersionsTest() {
  ...
  fun `test verifying stampfile`() = withTemp { path ->
    ...
    runTest { assertEquals(emptyList(), manager.verifyInstall(path.toString())) }
  }
  ...
  fun `test verifying stampfile with missing file`() = withTemp { path ->
    ...
    runTest { assertEquals(listOf("file2 does not exist"), manager.verifyInstall(path.toString())) }
  }
  ...
  fun `test verifying stampfile with invalid file`() = withTemp { path ->
    ...
    runTest { assertEquals(listOf("file2 is invalid"), manager.verifyInstall(path.toString())) }
  }
  ...
  fun `test generating stampfile`() = withTemp { path ->
    ...
    runTest { assertEquals(expected, manager.generateStampFile(path.toString())) }
  }
}
