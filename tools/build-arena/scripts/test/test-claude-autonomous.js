#!/usr/bin/env node
/**
 * Test script to watch Claude Code build autonomously
 * Starts a container, connects WebSocket, mirrors output to console
 */
...
console.log('\n🎬 Claude Code Autonomous Build Test\n');
...
async function connectWebSocket(containerId) {
  return new Promise((resolve, reject) => {
    ...
    ws.on('open', () => {
      ...
      setTimeout(() => {
        // Send Claude Code command with --print flag (non-interactive)
        const command = `claude --print --output-format json --max-turns 50 "Clone ${REPO_URL}, analyze the project structure, then build it using Elide. Time the build and report the results. Read /workspace/CLAUDE.md for instructions."\n`;

        console.log(`🤖 Sending command to Claude Code (non-interactive mode)...\n`);
      ...
      try {
        ...
        if (message.type === 'output') {
          ...
          if (output.includes('Build succeeded') ||
              ...
              output.includes('CLAUDE CODE EXECUTION COMPLETED')) {
            console.log('\n\n🏁 Build completed!');
            setTimeout(() => {
              ws.close();
              resolve();
...
async function main() {
  try {
    ...
    // Connect and watch
    await connectWebSocket(containerId);
