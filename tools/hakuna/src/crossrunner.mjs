...
const cmd = new Command("hakuna").name("hakuna").description(pkgInfo.description).version(pkgInfo.version)
...
export async function runBenchWithRuntime(runtime, path, benchmark) {
  switch (runtime) {
    ...
    case "elide":
      return await runBenchAndPipe(runtime, path, [
        "run",
        "--js",
        "--host:allow-all",
        "./tools/hakuna/src/runner.mjs",
        benchmark,
      ])
    case "node":
...
export default async function main() {
  const args = process.argv.slice(1).filter(arg => !(arg.includes("hakuna/src/entry.mjs") || arg.includes("$bunfs")))
